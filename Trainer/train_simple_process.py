...
from resnet import Simple
from aetad import AETAD_4Direct
...
def main():
    ...
    for epoch in range(config.start_epoch, config.epochs):
        train(base_model, model, train_dataloader, psnr_criterion, pixel_criterion, optimizer, epoch, scaler, writer)

        psnr = validate(base_model, model, valid_dataloader, psnr_criterion, epoch, writer)
...
def import_model() -> nn.Module:
    model = AETAD_4Direct()
    checkpoint = torch.load('./results/SRResNet_aeted/g-best.pth', map_location='cuda:0')
...
def train(base_model, model, train_dataloader, psnr_criterion, pixel_criterion, optimizer, epoch, scaler, writer) -> None:
...
def validate(base_model, model, valid_dataloader, psnr_criterion, epoch, writer) -> float:
    batch_time = AverageMeter("Time", ":6.3f")
    psnres = AverageMeter("PSNR", ":4.2f")
